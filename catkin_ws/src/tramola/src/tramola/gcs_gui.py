            ...
            try:
                ...
                try:
                    ...
                    if PIL_AVAILABLE:
                        ...
                        if self.callback and (current_time - self.last_callback_time) > 0.25:  # Max 4 redraws per second
                            self.callback()
...
class SimpleMapCanvas(tk.Canvas):
    ...
    def on_mouse_drag(self, event):
        """Handle mouse dragging to pan the map"""
        ...
        # The factor for how much lat/lon changes based on pixel movement
        # Higher zoom means smaller changes per pixel
        factor = 180.0 / (self.mercator_factor() / 256.0)
        
        # Convert pixel offset to lat/lon offset (with proper scaling)
        # For latitude, we need to adjust by the cosine of the latitude to account for mercator projection
        lat_offset = (dy * factor) / height
        lon_offset = (dx * factor) / width
        
        # Update the center point (minus because moving the map is the opposite of moving the view)
    ...
    def lat_lon_to_pixel(self, lat, lon):
        ...
        center_y = (1 - math.log(math.tan(center_lat_rad) + 1/math.cos(center_lat_rad)) / math.pi) / 2 * self.mercator_factor()
    ...
    def pixel_to_tile_position(self, tile_x, tile_y):
        ...
        pixel_x = width/2 + (tile_x - center_tile_x) * self.tile_size
        pixel_y = height/2 + (tile_y - center_tile_y) * self.tile_size
...
class GCSApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.lat_entry = ttk.Entry(waypoint_frame, width=15)
        ...
        self.lon_entry = ttk.Entry(waypoint_frame, width=15)
